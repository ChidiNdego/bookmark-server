...
def CheckURI(uri, timeout=5):
    ...
    if requests.get(uri,timeout=timeout).status_code == 200:
        return True
    elif requests.get(uri,timeout=timeout).status_code != 200:
        return False
    else:
        return 'Time Out...'
...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            if name in memory:
                ...
                self.end_headers()
                self.wfile.write("Sorry! I don't know '{}'.".format(name).encode())
        else:
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Please fill in the respective fields".encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Sorry! Couldn't fetch the URI {}. Please type a valid URI.".format(longuri).encode())
